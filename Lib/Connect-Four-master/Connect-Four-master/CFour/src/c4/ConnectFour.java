...
public class ConnectFour {
	...
	/**
	 * Copy-Constructor for ConnectFour. Only use, if necessary. Make sure, that
	 * the colHeight-Array contains the correct heights for each column,
	 ...
	 */
	public ConnectFour(long fieldP1, long fieldP2, int[] colHeight) {
	...
	/**
	 * The internal representation of the board are two BitBoards an array
	 * containing the heights of each column. Arrays have to be converted in
	 * this form
	 * 
	 * @param field
	...
	/**
	 * Like {@link ConnectFour#generateMoves(int)}, but without move-ordering.
	 * 
	 * @param sameLength
	 *            If sameLength = true, then always a moveList with 8 elements
	 *            is returned, where a value of -1 indicates the end of the
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < ConnectFour.fieldMask.length; i++)
			System.out.println(Arrays.toString(ConnectFour.fieldMask[i]));

		c4.computeFieldMasks();
		System.out.println("Length: " + ConnectFour.fieldMask.length);
		for (int i = 0; i < ConnectFour.fieldMask.length; i++)
			System.out.println(Arrays.toString(ConnectFour.fieldMask[i]));
	}
